...
class Tbl_juegos(models.Model):
    nombre = CharField(max_length=100)
    sinopsis = TextField()
    img = ImageField(upload_to='indieapp/images/') 
...
class Tbl_tres_populares(models.Model):
    ...
    img = ImageField(upload_to='indieapp/images/3_populares')
    posicion = CharField(max_length=25)

    def __str__(self):
        dimensiones = self.fnt_dimensiones(self.posicion)
        return f"{self.posicion} {dimensiones}"
    
    def fnt_dimensiones(self,posicion):
        self.dimensiones = {
            'inferior' : '680×230',
            'superior' : '680×230',
            'izquierda' : '680×430'
        ...
        return self.dimensiones.get(posicion)